...
由于{\dk}主要使用$ctex$宏包进行中文环境底层支持，字体采用custom方式设置，配置位置在：

\dkbutton{./src/ctex-fontset-custom.def}

采用这种方式的主要原因是$ctex$的字体套装在跨平台显示时效果和质量都会有区别，并且{\dk}额外添加了一些中文常用的字体，完整列表如下：
