...
[必选]
{
  \item 激活xcolor宏包的table选项，用于支持tabular环境的颜色配置。
  \item 由于Awesome-CV也引入了xcolor宏包，为了在无侵入的情况下避免配置冲突，需要在documentclass之前传递参数。
  \item 所有{\color{awesome}置顶配置}都必须出现在documentclass之前！
}
\cventry
[\Verb{\documentclass[12pt, a4paper, final]{awesome-cv}}]
[基本配置]
{
...
\cventry[个人信息部分][Awesome-CV][该部分用来定义一些个人信息或文档信息][部分可选]
{
  ...
  \item 该部分除了\Verb{\name}是必选的以外，其他设定不需要的均可以注释掉，首页中相应的部分会进行自适应。
